...
const handleMisses = ({ packages, results, dryRun }) => {
  ...
  if (unresolvedError && !dryRun) {
    throw new Error("Unresolved dynamic misses (see logs)");
  }
};
...
const createPackage = async ({ opts: { config, concurrency, report, dryRun } = {} } = {}) => {
  ...
  handleMisses({ packages: plan.packages, results, dryRun });
};
