...
const parseConfig = async ({
  ...
  const results = await Promise.all(Object.entries(packages).map(async ([pkgKey, pkgCfg]) => {
    ...
    // Collapsed options.
    const optsCollapsed = options.collapsed || {};
    const pkgCollapsed = pkgCfg.collapsed || {};
    ...
    return [pkgKey, {
      ...
      output,
      collapsed: {
        bail: typeof pkgCollapsed.bail !== "undefined"
          ? pkgCollapsed.bail
          : optsCollapsed.bail !== false
      },
      trace,
      traceOptions: {
        ...
        dynamic: {
          bail: typeof pkgDynamic.bail !== "undefined"
            ? pkgDynamic.bail
            : optsDynamic.bail === true,
