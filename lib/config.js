...
// Gather, merge, and normalize resolutions.
const getResolutions = (cwd, optsResolutions, pkgResolutions) =>
  ...
  const results = await Promise.all(Object.entries(packages).map(async ([pkgKey, pkgCfg]) => {
    ...
    return [pkgKey, {
      ...
      traceOptions: {
        ...
        dynamic: {
          resolutions: getResolutions(cwd, optsDynamic.resolutions, pkgDynamic.resolutions)
        }
      },
      include
    }];
