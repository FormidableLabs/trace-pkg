...
const parseConfig = async ({
  ...
  const results = await Promise.all(Object.entries(packages).map(async ([pkgKey, pkgCfg]) => {
    ...
    const resolutions = smartMerge(optsDynamic.resolutions, pkgDynamic.resolutions);
    Object.entries(resolutions)
      .filter(([resKey]) => resKey.startsWith("."))
      .forEach(([resKey, resVal]) => {
        // Replace key and mutate falsy values to an empty array.
        delete resolutions[resKey];
        resolutions[path.resolve(cwd, resKey)] = resVal || [];
      });
